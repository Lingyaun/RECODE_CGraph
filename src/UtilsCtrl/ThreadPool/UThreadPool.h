...
public:
    ...
    std::future<typename std::result_of<FunctionType()>::type> commit(FunctionType func) {
        ...
        if (!CGRAPH_FAIR_LOCK_ENABLE && cur_index_ >= 0 && cur_index_ < CGRAPH_DEFAULT_THREAD_SIZE) {//依次轮询放入核心线程队列，超过核心线程数时放入线程池队列（可以提供给辅助线程执行），超过最大线程数时从头循环
            // 部分任务直接放到线程的队列中执行
    ...
    CSTATUS init() override {
        ...
        primary_threads_.reserve(std::max(1,CGRAPH_DEFAULT_THREAD_SIZE));
