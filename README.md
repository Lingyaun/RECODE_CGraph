...
**认知：** 锁相关：`std::mutex`（互斥锁）与`std::shared_mutex`（读写锁）；`std::lock_guard`构造时加锁，离开作用域析构解锁，不支持手动解锁或重新加锁，无额外状态存储，开销更小 不可移动或复制。`std::unique_lock`用于代替互斥对象的成员函数，管理互斥对象，支持延迟加锁（defer_lock）、手动解锁（unlock()）和重新加锁（lock()），更灵活。适合需要条件变量或复杂锁管理的场景，需要维护锁状态（如是否已加锁），略有性能损耗，可通过移动语义转移锁的所有权。
